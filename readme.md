# Теоретическое описание Sigma-Pi сетей

Sigma-Pi сети (??-сети) — это тип искусственных нейронных сетей, в которых каждый нейрон вычисляет взвешенную сумму произведений входных сигналов. Название происходит от обозначений суммы (?) и произведения (?) в математике.
...
### Свойства слоя
- `Weights`: обучаемые веса, матрица размером `[num_neurons ? num_terms]`
- `degree`: максимальная степень полинома
- `num_neurons`: количество нейронов в слое
...
- Создание Sigma-Pi слоя с 2 входными каналами
- Максимальная степень полинома: 4
- Количество нейронов: 10
- Автоматический расчет количества полиномиальных членов (15 термов)
- Проверка прямого прохода через слой
